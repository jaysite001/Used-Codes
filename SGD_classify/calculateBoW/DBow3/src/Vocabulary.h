...
public:
   ...
   * @return descriptor
   */
  virtual  cv::Mat getWord(WordId wid) const;
   ...
   * @param filename
   */
//  void save(const std::vector<std::vector<cv::Mat> > &training_features, const std::string &filename, bool binary_compressed=true) const;
   ...
   */
//  virtual void save(const std::vector<std::vector<cv::Mat> > &training_features,cv::FileStorage &fs, 
...
public:
  ...
  virtual void transform_vec(const std::vector<cv::Mat>& features,std::vector<BowVector> &v, std::vector<FeatureVector> &fv, int levelsup, int vec_number) const;
    
  virtual void transform_vec2(const cv::Mat &feature,std::vector<WordId> &word_id_vec, std::vector<WordValue> &weight_vec, std::vector<NodeId> &nid_vec, int levelsup, int vec_number) const;
    
  virtual void transform_min_distance(const cv::Mat &feature,
    WordId &word_id1, double &distance1,WordId &word_id2, double &distance2 ) const;
    
    virtual void transform_min_distance3(const std::vector<cv::Mat>& features,
                                         std::vector<DBoW3::BowVector>& vec_bow, std::vector<DBoW3::FeatureVector>& vec_fet,
                                         int levelsup,int number = 3) const;
  /**
   * Returns the word id associated to a feature
   * @param feature
   * @param id (out) word id
