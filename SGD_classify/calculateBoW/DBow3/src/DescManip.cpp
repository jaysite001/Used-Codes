...
//            sqd += (a_ptr[i  ] - b_ptr[i  ])*(a_ptr[i  ] - b_ptr[i  ]);
        sqd = euclidean_float(a.cols,a_ptr,b_ptr);
    return sqd;
    }
}
...
//            std::cout<<a<<std::endl;
//            std::cout<<"load voc success"<<std::endl;
//            for(int i = 0; i <  a.cols; ++i)
//                if ( !(ss >> a.at<float>(i)))cerr<<"Error reading. Unexpected EOF. DescManip::fromString"<<endl;
        }
    }
