...
//        for(qit = ret.begin(); qit != ret.end(); qit++)
//            qit->Score = -qit->Score/2.0;
//        std::cout<<"searching for image_input_for_track "<<ret<<std::endl<<std::endl;
//    } // for each query word
...
void database_toStream(  std::ostream &out_str, bool compressed)
{
...
{
//    if(binary_compressed)
//    {
//        std::ofstream file_out(filename);
//        if (!file_out) throw std::runtime_error("Database::saveBinary Could not open file :"+filename+" for writing");
//        uint64_t sig=88877711233;//magic number describing the file
//        bool compressed=true;
//        file_out.write((char*)&sig,sizeof(sig));
//        file_out.write((char*)&compressed,sizeof(compressed));
////        m_voc->save(filename,binary_compressed);
////        std::stringstream aux_stream;
//        file_out.write((char*)&m_nentries,sizeof(m_nentries));
//        int m_use_di_i;
//        if(m_use_di)
//            m_use_di_i = 1;
//        else
//            m_use_di_i = 0;
//        file_out.write((char*)&m_use_di_i,sizeof(m_use_di_i));
//        file_out.write((char*)&m_dilevels,sizeof(m_dilevels));
//
//        for(auto iit = m_ifile.begin(); iit != m_ifile.end(); ++iit)
//        {
//            for(auto irit = iit->begin(); irit != iit->end(); ++irit)
//            {
//                file_out.write((char*)&(irit->entry_id),sizeof(irit->entry_id));
//                file_out.write((char*)&(irit->word_weight),sizeof(irit->word_weight));
//            }
//        }
...
//        int chunkSize=10000;
//        std::vector<char> db_compressed( chunkSize+size_t(400), 0);
...
//        file_out.write((char*)&nChunks, sizeof(nChunks));
//        //start compressing the chunks
//        while(total_size!=0){
//            int readSize=chunkSize;
//            if (total_size<chunkSize) readSize=total_size;
//            aux_stream.read(&input[0],readSize);
//            uint64_t  compressed_size   = qlz_compress(&input[0], &db_compressed[0], readSize, &state_compress);
//            total_size-=readSize;
//            file_out.write(&db_compressed[0], compressed_size);
